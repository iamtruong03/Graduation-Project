...
const ProjectHistory = ({ history = [] }) => {
  console.log('ProjectHistory component received:', history);
        ...
        {history.map((item, index) => {
          ...
          return (
            <TimelineItem key={item.id || index}>
              ...
              <TimelineSeparator>
                <TimelineDot variant="outlined" color={getStatusColor(item.newState)} />
                {index < history.length - 1 && <TimelineConnector />}
              </TimelineSeparator>
              <TimelineContent>
                <Typography variant="body2" component="span">
                  {item.stateName || getStatusName(item.newState)}
                </Typography>
